$(document).ready(function () {
    ...
    if (estPageQuiz) {
        // Récupération du nom de l'utilisateur dans l'URL
        const parametresURL = new URLSearchParams(window.location.search);
        nomUtilisateur = parametresURL.get('user');

        // Affichage du nom de l'utilisateur (ou "Utilisateur" par défaut)
        $('#userGreeting').text(nomUtilisateur || 'Utilisateur');

        // Chargement des données du quiz
        chargerDonneesQuiz();

        // Gestion du formulaire pour avancer dans les questions
